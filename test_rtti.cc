...
int main() {
    /*
    优秀的c++代码一定相对很少转型，把转型权利放给客户，自己尽量少转，最好用下面方式转型，而不是C风格转

    static_cast：用于进行静态类型转换，可以在不同但相关类型之间进行转换，
                如基本数据类型之间的转换、隐式转换（编译器可以自动执行的转换）的显式化、子类指针向上或向下转换到任意层次的父类指针等。
                但是，static_cast不能进行动态转换，即无法在运行时检查转换是否有效。
                可以将non-const对象转换为const对象，但是反过来不行。
                强迫隐式转换

    dynamic_cast：用于进行动态类型转换，用于多态类之间的转换，即在有继承关系的类之间进行转换。
                它将一个指向基类的指针或引用转换为指向派生类的指针或引用。dynamic_cast会在运行时检查转换的有效性，
                如果转换不合法，则返回空指针（对于指针转换）或抛出std::bad_cast异常（对于引用转换）。
                主要用于安全向下转换，但也是耗费重大运行成本的动作
                使用前务必谨慎

    const_cast：用于去除指针或引用的const和volatile限定符。
                它可以将一个常量指针或引用转换为非常量的指针或引用，以便可以修改被指向或引用的数据。
                但是，使用const_cast去除const限定符进行非法的写操作是不安全的，可能导致未定义行为。

    reinterpret_cast：用于进行底层的、不安全的类型转换，通常用于将一个指针转换为另一个不相关的类型的指针，
                或者将任何类型的指针转换为一个整数类型。reinterpret_cast对类型的转换没有任何安全性保障，潜在地会导致未定义行为。
    */
