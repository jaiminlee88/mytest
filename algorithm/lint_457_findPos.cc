...
public:
    ...
    int findPosition(vector<int> &nums, int target) {
        ...
        // return findPosition_core_nonrecurse_1(nums, target);
        return findPosition_core_nonrecurse_2(nums, target);
    }
    ...
    int findPosition_core_nonrecurse(vector<int> nums, int target) {
        ...
        else if (nums[rpos] == target) { // necessary？
            return rpos;
        }
        return  -1;
    }
    ...
    // 背下这个
    int findPosition_core_nonrecurse_1(vector<int> nums, int target) { // 找第一个target
        int lpos = 0;
        int rpos = nums.size() - 1;
        while (lpos + 1 < rpos) { // 最后一定合到两个或者一个数
            int m = (lpos + rpos) / 2;
            if (nums[m] < target) {
                lpos = m + 1;
    ...
    // 重要，背诵
    int findPosition_core_nonrecurse_2(vector<int> nums, int target) { // 找最后个target
        int lpos = 0;
        int rpos = nums.size() - 1;
        while(lpos + 1 < rpos) {
